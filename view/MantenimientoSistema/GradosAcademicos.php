        ...
        <main id="main-container">
            ...
            <div class="content">
                ...
                <div class="block-header block-header-default">
                        <h3 class="block-title">Grados Academicos <small></small></h3>
                    </div>

                <!DOCTYPE html>
<html>
<head>
    <title>Tabla de Grados Academicos</title>
    <style>
        table {
            width: 100%;
...
<div class="top-buttons">
    ...
    <form action="agregar.php" method="POST" style="display:inline;">
        <input type="submit" value="Agregar Grados Academicos">
    </form>
</div>
...
echo '<tr>';
echo '<th>IDgrado</th>';
echo '<th>Nombre Grado</th>';
echo '<th>Acción</th>';
echo '</tr>';

// Si hay departamentos, los mostramos
if (!empty($gradosacademicos)) {
    foreach ($gradosacademicos as $gradosacademicos) {
        echo '<tr>';
        echo '<td>' . $gradosacademicos['IDgrado'] . '</td>';
        echo '<td>' . $departamento['NombreGrado'] . '</td>';
        echo '<td class="action-buttons">';
        echo '<form action="editar.php" method="POST">';
        echo '<input type="hidden" name="IDgrado" value="' . $gradosacademicos['IDgrado'] . '">';
    ...
    echo '<td class="action-buttons">';
    echo '<form action="editar.php" method="POST">';
    echo '<input type="hidden" name="IDgrado" value="">';
    echo '<input type="submit" value="Editar">';
    echo '</form>';
    echo '<form action="eliminar.php" method="POST">';
    echo '<input type="hidden" name="IDgrado" value="">';
