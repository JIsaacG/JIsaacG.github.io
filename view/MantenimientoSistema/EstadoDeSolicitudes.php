        ...
        <main id="main-container">
            ...
            <div class="content">
                ...
                <div class="block-header block-header-default">
                        <h3 class="block-title">Estado de Solicitudes<small></small></h3>
                    </div>

                <!DOCTYPE html>
<html>
<head>
    <title>Tabla de Estado de Solicitudes</title>
    <style>
        table {
            width: 100%;
...
<div class="top-buttons">
    ...
    <form action="agregar.php" method="POST" style="display:inline;">
        <input type="submit" value="Agregar Estados">
    </form>
</div>
...
if (!empty($estadosolicitud)) {
    foreach ($estadosolicitud as $estadosolicitud) {
        echo '<tr>';
        echo '<td>' . $estadosolicitud['ID Estado'] . '</td>';
        echo '<td>' . $departamento['Descripcion'] . '</td>';
        echo '<td class="action-buttons">';
        echo '<form action="editar.php" method="POST">';
        echo '<input type="hidden" name="ID Estado" value="' . $estadosolicitud['ID Estado'] . '">';
    ...
    echo '<td class="action-buttons">';
    echo '<form action="editar.php" method="POST">';
    echo '<input type="hidden" name="ID Estado" value="">';
    echo '<input type="submit" value="Editar">';
    echo '</form>';
    echo '<form action="eliminar.php" method="POST">';
    echo '<input type="hidden" name="ID Estado" value="">';
