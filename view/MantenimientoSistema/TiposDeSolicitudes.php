...
<html>
<head>
    <title>Tabla de Tipos de Solicitudes</title>
    <style>
        table {
            width: 100%;
...
<div class="top-buttons">
    ...
    <form action="agregar.php" method="POST" style="display:inline;">
        <input type="submit" value="Agregar Solicitudes">
    </form>
</div>
...
echo '<th>Nombre Tipo Solicitud</th>';
echo '<th>Acción</th>';
echo '</tr>';

// Si hay departamentos, los mostramos
if (!empty($tiposolicitud)) {
    foreach ($tiposolicitud as $tiposolicitud) {
        echo '<tr>';
        echo '<td>' . $tiposolicitud['ID Tipo Solicitud'] . '</td>';
        echo '<td>' . $departamento['Nombre Tipo Solicitud'] . '</td>';
        echo '<td class="action-buttons">';
        echo '<form action="editar.php" method="POST">';
        echo '<input type="hidden" name="ID Tipo Solicitud" value="' . $tiposolicitud['ID Tipo Solicitud'] . '">';
    ...
    echo '<td class="action-buttons">';
    echo '<form action="editar.php" method="POST">';
    echo '<input type="hidden" name="ID Tipo Solicitud" value="">';
    echo '<input type="submit" value="Editar">';
    echo '</form>';
    echo '<form action="eliminar.php" method="POST">';
    echo '<input type="hidden" name="ID Tipo Solicitud" value="">';
